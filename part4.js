...
db.warehouse_district.aggregate([{"$unwind": "$district"}, { $group: { _id: null, total: {$sum: "$district.d_ytd"}}} ])
...
db.order.aggregate([{"$unwind": "$o_line"},  { $group: { _id: null, total: {$sum: "$o_line.ol_amount"}}} ])
db.order.aggregate([ {"$unwind": "$o_line"}, { $group: { _id: null, total: {$sum: "$o_line.ol_quantity"}}} ])
