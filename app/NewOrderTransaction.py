...
class NewOrderTransaction(object):
	...
	def getWarehouseInformation(self):
		...
		result = list(self.session.warehouse_district.aggregate(warehouseDistrictRowAggregate))
	...
	def updateStockItem(self, ytd, _remotecnt, s_qty, wid, iid):
		rows = self.stockitem.find({"s_w_id":wid, "s_i_id":iid})
		if rows:
